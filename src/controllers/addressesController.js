...
const get = async (req, res) => {
  try {
    ...
    if (!id) {
      let response = await Address.findAll({ where: { userId: user.id } });
    ...
    let response = await Address.findOne({ where: { id, userId: user.id } });
...
const create = async (dados, res, user) => {
  let { address, district, addressNumber, complement, city, state, zipCode } = dados;
